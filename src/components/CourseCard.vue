<template>
    <div class="course-card-container" v-bind:class="rootClassName">
      <svg viewBox="0 0 1024 1024" class="course-card-icon">
        <path
      ...
      <span class="course-card-text">{{name}}</span>
    </div>
  </template>
  ...
  .course-card-icon {
    fill: var(--dl-color-darkmode-base);
    width: 24px;
    height: 24px;
